...
(function() {
  'use strict';
  var BrunchWatcher, async, bindWatcherEvents, changeFileList, chokidar, debug, fs_utils, generateParams, getCompileFn, getPluginIncludes, getReloadFn, helpers, initWatcher, initialize, isPluginFor, logger, propIsFunction, sysPath, watch,
    __slice = [].slice,
  ...
  getPluginIncludes = function(plugins) {
    return plugins.map(function(plugin) {
      return helpers.callFunctionOrPass(plugin.include, plugin);
  ...
  initialize = function(options, configParams, onCompile, callback) {
    return helpers.loadPackages(helpers.pwd(), function(error, packages) {
      ...
      minifiers = plugins.filter(propIsFunction('minify')).concat(plugins.filter(propIsFunction('optimize')));
